exports.quotes = [
  {
    message: "Life isn’t about getting and having, it’s about giving and being.",
    name: "Kevin Kruse",
  },
  {
    message: "People often say that motivation doesn’t last. Well, neither does bathing. That’s why we recommend it daily.",
    name: "Zig Ziglar",
  },
  {
    message: "Dreaming, after all, is a form of planning.",
    name: "Gloria Steinem",
  },
  {
    message: "You become what you believe.",
    name: "Oprah Winfrey",
  },
  {
    message: "Start where you are. Use what you have. Do what you can.",
    name: "Arthur Ashe",
  },
  {
    message: "There is only one way to avoid criticism: do nothing, say nothing, and be nothing.",
    name: "Aristotle",
  },
  {
    message: "The mind is everything. What you think you become.",
    name: "Buddha",
  },
  {
    message: "I would rather die of passion than of boredom.",
    name: "Vincent van Gogh",
  },
];

exports.colors = ["dark", "info", "primary", "success", "danger", "warning", "secondary"];
